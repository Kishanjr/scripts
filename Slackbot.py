...
def handle_order_message(message, say, context):
    ...
    user_state[user_id] = {"order_number": order_number, "step": "issue_selection"}
...
def handle_button_click(ack, body, say):
    ...
    if user_id in user_state:
        user_state[user_id]["issue_type"] = issue_type
        user_state[user_id]["step"] = "awaiting_followup"

        followup_question = RESPONSES.get(issue_type, "Please explain the issue.")
        say(followup_question)
...
def handle_followup_message(message, say):
    ...
    if user_id in user_state:
        ...
        if state.get("step") == "awaiting_followup":
            ...
            # Simulate saving the reply or sending to support
            say(f"Thank you! We've recorded your issue: *{issue_type.replace('_', ' ').title()}*\nYour message: *{text}*\nOur team will follow up on order #{order_number}.")

            # Clear state
            del user_state[user_id]
        else:
            say("Please provide your order number (e.g., order #12345) to get started.")
    else:
        say("Please provide your order number (e.g., order #12345) to begin.")
